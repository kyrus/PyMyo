...
##Rich mod - mske this function more abstract
def crackHash_quiet (algorithm, hashvalue, output):
	"""Crack a hash or all the hashes of a file.
	
	@param alg Algorithm of the hash (MD5, SHA1...).
	@param hashvalue Hash value to be cracked.
	@param output Function to supply progress feedbak to caller
	@return dictionary of hash/cracked value pairs"""
	...
	for activehash in hashestocrack:
		...
		for i in range(len(CRAKERS)):
			...
			# Analyze the hash
			output( "Analyzing with %s (%s)..." % (cr.name, cr.url) )
		...
		if hashresults:
			...
			# Valid results are stored
			#crackedhashes.append ( (activehash, finalresult) )
			crackedhashes[activehash] = [ finalresult, cr.name ]
